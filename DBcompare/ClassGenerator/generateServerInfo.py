...
def generate_csharp_class(json_data, class_name, additional_usings=None):
    ...
    csharp_class += f"        {class_name} ServerInfo = JsonConvert.DeserializeObject<{class_name}>(File.ReadAllText(configurationPath));\n"
    csharp_class += "\n"
    csharp_class += f"        Console.WriteLine($\"{class_name} Refreshed\");\n"
    csharp_class += "\n"
    csharp_class += f"        Instance = ServerInfo;\n"
    csharp_class += "    }\n"
...
json_file_path = os.path.expanduser("~/Project/DBcompare/DBcompare/DBcompare/ServerInfo.config")
...
output_file_path = os.path.expanduser("~/Project/DBcompare/DBcompare/ClassGenerator/Generated/ServerInfo.cs")
